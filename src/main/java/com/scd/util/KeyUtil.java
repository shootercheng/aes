...
public class KeyUtil {
	...
	public static String genRandomKey(int keyLength) {
		...
		for (int i = 0; i < keyLength; i++) {
			// 产生随机二位数组索引 0-2
			int index = SecureRandomHolder.random.nextInt(3);
			switch (index){
				case 0:
				    // 第一个二位数组索引
				    int index0 = SecureRandomHolder.random.nextInt(ENCRYPT_ELEMENTS[0].length);
                    strKey = ENCRYPT_ELEMENTS[0][index0];
                    key.append(strKey);
					break;
				case 1:
				    // 第二个二位数组索引
					int index1 = SecureRandomHolder.random.nextInt(ENCRYPT_ELEMENTS[1].length);
					strKey = ENCRYPT_ELEMENTS[1][index1];
					key.append(strKey);
					break;
				case 2:
					// 第三个二位数组索引
                    int index2 = SecureRandomHolder.random.nextInt(ENCRYPT_ELEMENTS[2].length);
					strKey = ENCRYPT_ELEMENTS[2][index2];
					key.append(strKey);
					break;
				default:
					break;
